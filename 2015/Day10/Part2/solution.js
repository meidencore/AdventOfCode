...
Starting with the digits in your puzzle input, reapply this process 50 times.
What is the length of the result?
*/

const seed = "1113122113";

/**
 * @param {string} seed
 * @param {number} repeat
 */
function lookAndSay(seed, repeat) {
  console.log("look", seed);
  let newSeed = "";
  let digit = seed[0];
  let copy = 1;
  ...
  for (let i = 1; i < seed.length; i++) {
    if (digit !== seed[i]) {
      newSeed = newSeed + String(copy) + digit;
      digit = seed[i];
      copy = 1;
    } else {
      copy++;
  ...
  newSeed = newSeed + String(copy) + digit;
  repeat--;
  console.log("say", newSeed);
  return repeat ? lookAndSay(newSeed, repeat) : newSeed;
}

console.log(lookAndSay(seed, 50).length);
